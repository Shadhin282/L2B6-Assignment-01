...
const filterByRating = (Value: Item[]):Item[] => {
    const filterRating = Value.filter(item => item.rating >= 4 && item.rating<=5)
    return filterRating;
}
...
const getUniqueValues = (value1: (string | number)[], value2: (string | number)[]): (string| number)[]=> {

    const uniqueValue: (string | number)[] = [];

    const checkUniqueValue = (array: (string | number)[], value: (string | number)): boolean => {
...
const calculateTotalPrice = (products: Product[]): number => {
    const totalPrice = products.map(product => {
        ...
        return product.discount ? priceofProduct * (1 - product.discount / 100) : priceofProduct;
    });
